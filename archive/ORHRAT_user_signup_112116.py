...
# If the user enters data properly:
        if (username and password and verify and firstname and lastname and organization and email):
            ...
            # Put info into database after making sure the username is not taken:
...
# ***** PUT THE HASHING FUNCTIONS HERE *****
...
# Key Name (username) should be the primary key:
...
class LoginPage(Handler):
    ...
    def post(self):
        ...
        if uc:
            ...
            # Is this more clear? And is the last line correct?:
            # salt_query = db.GqlQuery("SELECT salt FROM Users WHERE username = :1", un)
            # salt_result = salt_query.get()
            ...
            if pw_hash == p:
                ...
                self.render('login.html', username = '', password = '', login_error = login_error)



# This shows the long version of authenticating cookies. The short version (with check_cookies) isn't much shorter.
...
class WelcomePage(Handler):
    def get(self):
        ...
        # It's possible that this security feature is not necessary for every page:
...
# Now it appears to be redundant -- same as check_secure_val.
def check_cookie(name_hash):
    name = name_hash.split('|')[0]
    cookie_val = check_secure_val(name_hash)
    return cookie_val
...
class Standard4Page(Handler):
    ...
    def post(self):
        ...
        if cc == None:
            ...
            # Look into name="st4" for all checkboxes.
            # For Standard 4, this would be a list of 21 checkboxes:
            ...
            self.render("standard_4.html", **checkboxes)
...
# , ('/st4_checkboxes.html', Standard4Page)